...
import com.pillowsdk.demo.AutoStartActivity;
import com.pillowsdk.demo.MainActivity;
import com.pillowsdk.demo.R;
import com.pillowsdk.demo.RawDataActivity;
...
//		SdkLog.log(TAG+" onCreateView-----------");
	...
	protected void initUI() {
		...
		btnCollectStatus.setEnabled(getPillowHelper().isConnected());
		btnAutoStart.setEnabled(getPillowHelper().isConnected());
		btnStartCollect.setEnabled(getPillowHelper().isConnected());
	...
	public void onClick(View v) {
		...
		if(v == btnStartRealtimeData){
			...
			getPillowHelper().startRealTimeData(1000, realtimeCB);
		}else if(v == btnStopRealtimeData){
			printLog(R.string.stopping_data);
			getPillowHelper().stopRealTimeData(1000, new IResultCallback<Void>() {
				@Override
			...
			getPillowHelper().stopCollection(1000, new IResultCallback<Void>() {
				@Override
				public void onResultCallback(final CallbackData<Void> cd) {
					// TODO Auto-generated method stub
					mActivity.runOnUiThread(new Runnable() {
...
//								tvCollectStatus.setText(R.string.working_state_not);
			...
			getPillowHelper().getCollectionStatus(1000, new IResultCallback<Byte>() {
				@Override
			...
			getPillowHelper().startCollection(1000, new IResultCallback<Void>() {
				@Override
				public void onResultCallback(final CallbackData<Void> cd) {
					// TODO Auto-generated method stub
					mActivity.runOnUiThread(new Runnable() {
				...
				mActivity.runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						if(checkStatus(cd)){
							...
							}/*else if(cd.getType() == IMonitorManager.TYPE_SLEEP_STATUS){
